 ...
 * QNS-CD-1.0 companion (cite/proxy only; local qnsd is not hosted here).
...
export const QNS_CD = Object.freeze({
  ...
  runtime_design: "https://github.com/AzielEliab/aziel-runtime/blob/main/docs/designs/QNS-CD-1.0.md",
  note: "Photon QNS1 vias on local qnsd (127.0.0.1). This Worker cites/proxies mesh only. Restriction walks next via class. Author Aziel Eliab.",
});
...
export const MESH_PRODUCT = "azbot";
...
export const MESH_PROXY_ROUTES = Object.freeze([
  { path: MESH_PATH, methods: ["get", "head"], op: "status", summary: "PROXY to aziel-runtime GET /v1/mesh. Suite mesh status. Default OFF. QNS-CD-1.0 companion. Not a local op. Not local qnsd HTTP." },
...
export function meshPointer() {
  return {
    ...
    origin: RUNTIME + MESH_PATH,
    note: "PROXY to aziel-runtime /v1/mesh/* via AZIEL_RUNTIME. Not a local op. Not AnonBroadcast. Not AZMail's product-local ring. AZBot remains a skill, not a foundation model. Full node process is local qnm-node/. " + MESH_NOTE,
...
export function meshOpenApiPaths() {
  ...
  for (const route of MESH_PROXY_ROUTES) {
    ...
    for (const method of route.methods) {
      entry[method] = {
        operationId: "azbot_mesh_" + route.op + (method === "head" ? "_head" : "") + "_proxy",
...
function meshService(env) {
  const bind = env && env.AZIEL_RUNTIME;
  if (bind && typeof bind === "object" && typeof bind.fetch === "function") return bind;
  return null;
}

/** Stamp this Worker's QNS-CD-1.0 cite onto a mesh JSON envelope. Not a local qnsd door. */
 ...
 * Prefer env.AZIEL_RUNTIME service binding. HTTP fallback when unbound or 503.
 */
export async function originFetch(env, pathAndQuery, init) {
  ...
  headers.set("X-Aziel-Runtime-Via", "azbot-download-tracker");
  ...
  try {
    ...
    return {
      ...
      data: meshErrFields({
        ...
        http_status: null,
        content_type: "",
        via: meshService(env) ? "service-binding" : "http",
